...
const SortModal = () => {
  ...
  const showMenu = () => {
    const SortableItem = SortableElement(({ value }) => (
      <MenuItem style={{ zIndex: 10, cursor: 'all-scroll' }} ><Icon height={"0.6rem"} as={Drag} />&nbsp;<span title={value[settings.title]}>{shortenString(value[settings.title])}</span><span> --- </span><span>{value[settings.section]}</span></MenuItem>
    ));
